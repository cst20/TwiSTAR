...
# Usage: ./eval_parallel_8gpu_cot.sh [MODEL_PATH] [RUN_SUFFIX] [EXTRA_ARGS]

echo "[INFO] Starting 8-GPU Parallel CoT Evaluation..."

MERGED_MODEL_PATH="${1:-../train/results/ReasoningActivation/epoch_2}"
if [ "$#" -gt 0 ]; then
    shift
fi

RUN_SUFFIX="${1:-$(basename "$MERGED_MODEL_PATH")}" 
if [ "$#" -gt 0 ]; then
    shift
fi
...
LOG_DIR="logs_0930/parallel_cot_eval_${TS}_${RUN_SUFFIX}"
mkdir -p "$LOG_DIR"

echo "[INFO] Log directory: $LOG_DIR"
echo "[INFO] Started at: $(date)"
echo "[INFO] Run suffix: $RUN_SUFFIX"

TOTAL_SAMPLES=${TOTAL_SAMPLES:--1}
NUM_GPUS=${NUM_GPUS:-8}
BATCH_SIZE=${BATCH_SIZE:-4}
NUM_THINKING_SAMPLES=${NUM_THINKING_SAMPLES:-5}
NUM_BEAMS_PER_SAMPLE=${NUM_BEAMS_PER_SAMPLE:-10}
THINK_TOKENS=${THINK_TOKENS:-128}
SID_TOKENS=${SID_TOKENS:-8}
PRINT_GENERATIONS=${PRINT_GENERATIONS:-1}
PRINT_GENERATIONS_LIMIT=${PRINT_GENERATIONS_LIMIT:--1}
PRINT_GENERATIONS_GPU=${PRINT_GENERATIONS_GPU:--1}

if [ "$TOTAL_SAMPLES" -lt 0 ]; then
    TOTAL_SAMPLES=$(python3 - <<'PY'
import pandas as pd
print(len(pd.read_parquet('../data/training_RA_test.parquet')))
PY
)
fi
...
if [ "$PRINT_GENERATIONS" = "1" ]; then
    if [ "$PRINT_GENERATIONS_GPU" = "-1" ]; then
        echo "  Print generations: ENABLED on ALL GPUs (limit=${PRINT_GENERATIONS_LIMIT})"
    else
        echo "  Print generations: ENABLED on GPU ${PRINT_GENERATIONS_GPU} (limit=${PRINT_GENERATIONS_LIMIT})"
    fi
else
...
for gpu_id in $(seq 0 $((NUM_GPUS-1))); do
    ...
    echo "[INFO] Starting CoT GPU $gpu_id: shard_mode=stride, total_samples=${TOTAL_SAMPLES}, rank=${gpu_id}/${NUM_GPUS}"

    extra_print_args=()
    if [ "$PRINT_GENERATIONS" = "1" ] && { [ "$PRINT_GENERATIONS_GPU" = "-1" ] || [ "$gpu_id" -eq "$PRINT_GENERATIONS_GPU" ]; }; then
        extra_print_args+=(--print_generations --print_generations_limit ${PRINT_GENERATIONS_LIMIT})
    fi
...
with open(summary_log, 'w', encoding='utf-8') as f:
            ...
            try:
                ...
                if marker_pos != -1:
                    ...
                    if gpu_metrics:
                        ...
                        for metric in hit_metrics:
                            total_metrics[metric] += gpu_metrics.get(metric, 0.0) * samples
    ...
    if found_gpus > 0 and total_samples > 0:
        avg_metrics = {m: total_metrics[m] / total_samples for m in hit_metrics}
